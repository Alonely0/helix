...
fn ws_chars(config: &mut WhitespaceConfig, option: HashMap<SteelVal, char>) -> anyhow::Result<()> {
    ...
    //             "space" => if let WhitespaceRender::Specific { ref mut space, .. } = config.render { *space = Some(v) },
    //             "tab" => config.render.tab() = v,
    //             "nbsp" => config.render.nbsp() = v,
    //             "nnbsp" => config.render.nnbsp() = v,
    //             "newline" => config.render.newline() = v,
    //             "tabpad" => config.render.tabpad() = v,
